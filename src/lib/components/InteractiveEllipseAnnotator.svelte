...
<script lang="ts">
	...
	// This is a direct reference to the component instance passed from the parent
	...
	$: if (annotationImage) {
		initCanvasAndEllipse();
	}

	function initCanvasAndEllipse() {
		if (!canvas || !annotationImage) return;
		...
		img.onload = () => {
			...
			// Initialize ellipse in the center
			ellipse = {
				cx: canvas.width / 2,
				cy: canvas.height / 2,
				rx: canvas.width / 4,
	...
	function getHandlePositions() {
		...
		return {
			...
			rotate: { x: cx + rx * cosA * 1.2, y: cy + rx * sinA * 1.2 } // Outside the rx handle
		};
	}
	...
	function handleMove(event: MouseEvent | TouchEvent) {
		...
		if (activeHandle === 'center') {
			...
			const initialAngle = Math.atan2(
				dragStart.y - initialEllipse.cy,
				dragStart.x - initialEllipse.cx
			);
			...
			// Handle resizing from center, requires transforming coords to ellipse space
	...
	function handleAnalyze() {
		if (imageProcessor) {
			// Pass the final ellipse state to the processor and trigger analysis
			updateEllipsePriors(ellipse);
